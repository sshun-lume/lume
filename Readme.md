...
ルーメ（lúmë）はクウェンヤで時を表す語です。


## 何ができるのか

mlflowの機能で温度・エネルギーなど状態変数をメトリクスとして記録できたり
粒子のトラジェクトリや継続計算用の状態保存・シミュレーション計算後に作成したグラフなどをアーティファクトとして保存できたりします。
また、編集可能なdescription領域も持っているので適宜説明・注釈を追記できます。
シミュレーション計算のRUNを番号を振って台帳管理するようなことは不要になります。
計算に使ったパラメータから実行環境・日時・計算時間まで記録に残り何年後でも確認することができます。
大量の出力ファイルもグラフや計算の記録とセットで管理され、オブジェクトストレージのような柔軟で経済的な領域に蓄積できます。
...
mlflowとjupyterlabはPythonのvirtualenv環境で`pip install mlflow nvidia-ml-py`, `pip install jupyterlab jupyterlab-git`でインストールします。
